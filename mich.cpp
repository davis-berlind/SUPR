...
double elbo_fn(int T, int B_l, int B_r, double mu_0, double lambda_0,
  ...
  for (int t = 0; t < T; t++) {
    for (int j = 0; j < J; j++) {
      ...
      // mean component
      elbo += 0.5 * pi_bar_j(t,j) * (std::log(tau_j[j]) - std::log(tau_bar_j(t,j)));
      elbo += 0.5 * pi_bar_j(t,j) * (0.5 - tau_j[j] * (std::pow(b_bar_j(t,j), 2) * (u_bar_j(t,j) / v_bar_j(t,j)) + 1 / tau_bar_j(t,j)));
      // // precision component
      elbo += pi_bar_j(t,j) * (u_j[j] * (std::log(v_j[j]) - std::log(v_bar_j(t,j))) + std::lgamma(u_bar_j(t,j)) - std::lgamma(u_j[j]));
      elbo += pi_bar_j(t,j) * ((u_j[j] - u_bar_j(t,j)) * R::digamma(u_bar_j(t,j)) + u_bar_j(t,j) * (1 - v_j[j] / v_bar_j(t,j)));
    }
    for (int l = 0; l < L; l++) {
      // E[log p_l] - E[log q_l]
      ...
      elbo += 0.5 * pi_bar_l(t,l) * (std::log(tau_l[l]) - std::log(tau_bar_l(t,l)));
      elbo += 0.5 * pi_bar_l(t,l) * (0.5 - tau_l[l] * (std::pow(b_bar_l(t,l), 2) + 1 / tau_bar_l(t,l)));
    }
    for (int k = 0; k < K; k++) {
      ...
      elbo += pi_bar_k(t,k) * (u_k[k] * (std::log(v_k[k]) - std::log(v_bar_k(t,k))) + std::lgamma(u_bar_k(t,k)) - std::lgamma(u_k[k]));
      elbo += pi_bar_k(t,k) * ((u_k[k] - u_bar_k(t,k)) * R::digamma(u_bar_k(t,k)) + u_bar_k(t,k) * (1 - v_k[k] / v_bar_k(t,k)));
    }
  }
  return elbo;
}
...
List smcp(NumericVector y, NumericVector lambda, double tau, 
  ...
  for (int t = T + B_r - 1; t >= 0; t--) {
    ...
    if (t < T) {
      tau_bar[t] = tau + ls;
      mu[t] = ys / tau_bar[t];
      pi_bar[t] = log_pi[t] - 0.5 * (std::log(tau_bar[t]) - tau_bar[t] * std::pow(mu[t], 2.0));
...
List sscp(NumericVector y, NumericVector tau, double u, 
  ...
  return List::create(_["v"] = v_bar, _["pi"] = pi_bar / tot);
}
...
List smscp(NumericVector y, NumericVector lambda, 
  ...
  return List::create(_["mu"] = mu, _["tau"] = tau_bar, _["v"] = v_bar, _["pi"] = pi_bar / tot);
}

// [[Rcpp::export]]
List mich_cpp(NumericVector y_0, NumericVector y, int J, int L, int K, double mu_0, double lambda_0,
              ...
              NumericMatrix mu_lambda_j, NumericMatrix mu2_lambda_j, NumericMatrix lambda_bar_j, 
              NumericVector tau_l, NumericMatrix log_pi_l,
              NumericMatrix pi_bar_l, NumericMatrix b_bar_l, NumericMatrix tau_bar_l, 
              NumericMatrix mu_bar_l, NumericMatrix mu2_bar_l,
              NumericVector u_k, NumericVector v_k, NumericMatrix log_pi_k,
              NumericMatrix pi_bar_k, NumericMatrix u_bar_k, NumericMatrix v_bar_k,
              NumericMatrix lambda_bar_k) {
  ...
  List smcp_fit, sscp_fit, smscp_fit;
  ...
  while (iter < max_iter) {
    ...
    for (int j = 0; j < J; j++) {
      ...
      // fit single smscp model on residuals
      smscp_fit = smscp(r_tilde, lambda_bar, tau_j[j], u_j[j], v_tilde + v_j[j], log_pi_tilde + log_pi_j(_,j), B_r);
      
      // store updated prior parameters
      pi_bar_j(_,j) =  as<NumericVector>(smscp_fit["pi"]);
      b_bar_j(_,j) =  as<NumericVector>(smscp_fit["mu"]);
      tau_bar_j(_,j) =  as<NumericVector>(smscp_fit["tau"]);
      v_bar_j(_,j) =  as<NumericVector>(smscp_fit["v"]);
    ...
    for (int l = 0; l < L; l++) {
      ...
      // fit single smcp model on residuals
      smcp_fit = smcp(r_tilde, lambda_bar, tau_l[l], log_pi_l(_,l), B_r); 
      
      // store updated prior parameters
      pi_bar_l(_,l) =  as<NumericVector>(smcp_fit["pi"]);
      b_bar_l(_,l) =  as<NumericVector>(smcp_fit["mu"]);
      tau_bar_l(_,l) =  as<NumericVector>(smcp_fit["tau"]);
    ...
    for (int k = 0; k < K; k++) {
      ...
      // fit single sscp model on residuals
      sscp_fit = sscp(r_tilde, lambda_bar, u_k[k], v_tilde + v_k[k], log_pi_tilde + log_pi_k(_,k), B_r);
      
      // store updated prior parameters
      pi_bar_k(_,k) = as<NumericVector>(sscp_fit["pi"]);
      v_bar_k(_,k) = as<NumericVector>(sscp_fit["v"]);
