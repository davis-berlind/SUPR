...
mich <- function(y, J = 0, L = 0, K = 0, fit_intercept = FALSE, fit_scale = FALSE,
  ...
  if (sum(J_auto, L_auto, K_auto) == 1) {
    while (TRUE) {
      ...
      if (J_auto) {
        ...
        # fit new model 
        fit <- mich(y, J, L, K, fit_intercept, fit_scale,
        ...
        if (fit$elbo[length(fit$elbo)] < elbo) {
          if(verbose) print(paste0("J = ", J, ": ELBO = ",  fit$elbo[length(fit$elbo)], ". Done."))
          J <- J - 1
      ...
      if (L_auto) {
        ...
        # if elbo decreases return previous model
        if (fit$elbo[length(fit$elbo)] < elbo) {
          if(verbose) print(paste0("L = ", L, ": ELBO = ", elbo, ". Done."))
          L <- L - 1
          tau_l <- tau_l[-1]
      ...
      if (K_auto) {
        ...
        # if elbo decreases return previous model
        if (fit$elbo[length(fit$elbo)] < elbo) {
          if(verbose) print(paste0("K = ", K, ": ELBO = ", elbo, ". Done."))
          K <- K - 1
          u_k <- u_k[-1]; v_k <- v_k[-1];
  ...
  if (sum(J_auto, L_auto, K_auto) > 1) {
    ...
    while (TRUE) {
      ...
      # extract elbo
      elbo <- elbo_new[which.max(elbo_new)]
    }
  }
