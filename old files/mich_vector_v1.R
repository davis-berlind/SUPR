    ...
    while ((J < J_max | L < L_max | K < K_max) & counter > 0) {
      ...
      while (!merged) {
        ...
        if (L > 1 & !(L %in% merged_at_l)) {
          ...
          mu_bar_l <- fit_new$mean_model$mu_bar
          mu2_bar_l <- fit_new$mean_model$mu2_bar
          merge_residual <- fit_new$residual
          merge_lambda <- fit_new$lambda
          merge_delta <- fit_new$delta
        ...
        if (K > 1 & !(K %in% merged_at_k)) {
          ...
          lambda_bar_k <- fit_new$var_model$lambda_bar
          merge_residual <- fit_new$residual
          merge_lambda <- fit_new$lambda
          merge_delta <- fit_new$delta
        ...
        if (J > 1 & !(J %in% merged_at_j)) {
          ...
          mu_lambda_bar_j <- fit_new$meanvar_model$mu_lambda_bar
          mu2_lambda_bar_j <- fit_new$meanvar_model$mu2_lambda_bar
          lambda_bar_j <- fit_new$meanvar_model$lambda_bar
          merge_residual <- fit_new$residual
          merge_lambda <- fit_new$lambda
          merge_delta <- fit_new$delta
