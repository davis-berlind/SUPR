mich_vector <- function(y, fit_intercept, fit_scale,
  ...
  while (TRUE) {
    ...
    fit <- mich_cpp(y, J, L, K, mu_0, lambda_0,
                    ...
                    max_iter, verbose = verbose & !(L_auto | K_auto | J_auto), tol, 
    ...
    if (K > 1) {
      ...
      merge_mat <- (t(pi_bar_k) %*% pi_bar_k > merge_prob * merge_seq^cntr) & keep_mat
      ...
      if (any(merge_mat[lower.tri(merge_mat)])) {
        ...
        for (i in 1:(K-1)) {
          for (j in (i+1):K) {
            if (merge_mat[i, j]) {
              ...
              v_bar_k[,i] <- v_bar_k[,i] * v_bar_k[,j] / u_bar_k
              v_bar_k[,j] <- u_bar_k
            }
          }
        }
        ...
        log_pi_bar_k <- log_pi_bar_k[,order_k]
    ...
    if (no_merges) break
    cntr <- cntr + 1
    if (verbose & !(L_auto | K_auto | J_auto)) print(paste("Merging duplicate components: iteration", cntr))
  }
  ...
  if (reverse){
    ...
    if (!pi_l_weighted) {
      pi_l <- pi_l[1:T,1:max(1,L),drop = FALSE]
      log_pi_l <- log_pi_l[1:T,1:max(1,L),drop = FALSE]
    }
    if (!pi_k_weighted) {
      pi_k <- pi_k[1:T,1:max(1,K),drop = FALSE]
      log_pi_k <- log_pi_k[1:T,1:max(1,K),drop = FALSE]
    }
    if (!pi_j_weighted) {
