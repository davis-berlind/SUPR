mich_vector_rand <- function(y, fit_intercept, fit_scale,
  ...
  #### fit model ####
  fit <- mich_cpp_rand(y_0, y, J, L, K, mu_0, lambda_0,
  ...
  if (sum(J_auto, L_auto, K_auto) == 1) {
    ...
    while (TRUE) {
      ...
      # fit new model 
      fit_new <- mich_cpp_rand(y_0, y, J, L, K, mu_0, lambda_0,
                          B_l, B_r, fit_intercept, fit_scale, 
  ...
  if (sum(J_auto, L_auto, K_auto) > 1) {
    ...
    counter <- (J_auto + L_auto + K_auto) * ceiling(log(T) / 2) 
    elbo <- fit$elbo[length(fit$elbo)] # current value of elbo
