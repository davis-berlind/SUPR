...
double elbo_fn(int T, int B_l, int B_r, double mu_0, double lambda_0,
               ...
               NumericMatrix b_bar_j, NumericMatrix omega_bar_j, NumericMatrix u_bar_j, NumericMatrix v_bar_j, NumericMatrix pi_bar_j,
               double omega_j, double u_j, double v_j, NumericMatrix log_pi_j,
               NumericMatrix b_bar_l, NumericMatrix omega_bar_l, NumericMatrix pi_bar_l, 
  ...
  for (int t = 0; t < T; t++) {
    for (int j = 0; j < J; j++) {
      ...
      // mean component
      elbo += 0.5 * pi_bar_j(t,j) * (std::log(omega_j) - std::log(omega_bar_j(t,j)));
      elbo += 0.5 * pi_bar_j(t,j) * (1 - omega_j * (std::pow(b_bar_j(t,j), 2) * (u_bar_j(t,j) / v_bar_j(t,j)) + 1 / omega_bar_j(t,j)));
      // // precision component
    ...
    for (int l = 0; l < L; l++) {
      ...
      elbo += 0.5 * pi_bar_l(t,l) * (std::log(omega_l) - std::log(omega_bar_l(t,l)));
      elbo += 0.5 * pi_bar_l(t,l) * (1 - omega_l * (std::pow(b_bar_l(t,l), 2) + 1 / omega_bar_l(t,l)));
    }
    for (int k = 0; k < K; k++) {
...
NumericVector mu2_bar_fn(NumericVector b, NumericVector omega, 
  ...
  for (int t = 0; t < T + B_r; t++) {
    if (t < T) {
      fwd_sum += (std::pow(b[t], 2) + 1 / omega[t]) * prob[t];
      mu2_bar[t] = fwd_sum;
    } else {
...
NumericVector mu2_lambda_fn(NumericVector b, NumericVector omega, NumericVector u,
  ...
  for (int t = 0; t < T + B_r; t++) {
    if (t < T) {
      fwd_sum += (std::pow(b[t], 2) * (u[t] / v[t]) + 1 / omega[t]) * prob[t];
...
List mean_scp(NumericVector y, NumericVector lambda, double omega, 
  ...
  int T = y.length() - B_r;
  NumericVector b_bar (T), pi_bar (T), omega_bar (T);
  ...
  for (int t = T + B_r - 1; t >= 0; t--) {
    ...
    if (t < T) {
      ...
      pi_bar[t] = log_pi[t] - 0.5 * (std::log(omega_bar[t]) - omega_bar[t] * std::pow(b_bar[t], 2.0));
  ...
  return List::create(_["b_bar"] = b_bar, _["omega_bar"] = omega_bar, _["pi_bar"] = pi_bar / tot);
}

// [[Rcpp::export]]
List var_scp(NumericVector y, NumericVector omega, double u, 
  ...
  for (int t = T + B_r - 1; t >= 0; t--) {
    ...
    if (t < T) {
      ...
      fs += std::pow(y[T-t-1], 2.0) * omega[T-t-1];
    }
  }
...
List meanvar_scp(NumericVector y, NumericVector lambda, 
                 double omega, double u, NumericVector v, 
                 NumericVector log_pi, int B_r) {
  
  int T = y.length() - B_r;
  double half = T / 2.0;
  NumericVector b_bar (T), pi_bar (T, 0.0), u_bar (T), v_bar (T), omega_bar (T);
  ...
  for (int t = T + B_r - 1; t >= 0; t--) {
    ...
    if (t < T) {
      omega_bar[t] = omega + ls;
      b_bar[t] = ys / omega_bar[t];
      v_bar[t] = v[t] + 0.5 * (rs - std::pow(b_bar[t], 2.0) * omega_bar[t]);
      u_bar[t] = u + 0.5 * (T + B_r - t);
      pi_bar[t] += log_pi[t] + std::lgamma(u_bar[t]) - u_bar[t] * std::log(v_bar[t]) - 0.5 * std::log(omega_bar[t]);
      pi_bar[T-t-1] -= 0.5 * fs;
      if (t < half) {
        if (pi_bar[T-t-1] > log_pi_max) log_pi_max = pi_bar[T-t-1];
        if (pi_bar[t] > log_pi_max) log_pi_max = pi_bar[t];
...
List mich_cpp(NumericVector y_0, NumericVector y, 
              ...
              double omega_j, double u_j, double v_j, NumericMatrix log_pi_j,
              NumericMatrix pi_bar_j, NumericMatrix b_bar_j, NumericMatrix omega_bar_j, NumericMatrix u_bar_j, NumericMatrix v_bar_j,
              double omega_l, NumericMatrix log_pi_l,
              NumericMatrix pi_bar_l, NumericMatrix b_bar_l, NumericMatrix omega_bar_l, 
              double u_k, double v_k, NumericMatrix log_pi_k,
  ...
  if (refit) {
    for (int j = 0; j < J; j++) {
      ...
      mu2_lambda_j(_,j) = mu2_lambda_fn(b_bar_j(_,j), omega_bar_j(_,j), u_bar_j(_,j), v_bar_j(_,j), pi_bar_j(_,j), B_r);
  ...
  while (iter < max_iter) {
    ...
    for (int j = 0; j < J; j++) {
      ...
      meanvar_scp_fit = meanvar_scp(r_tilde, lambda_bar, omega_j, u_j, v_tilde + v_j, log_pi_tilde + log_pi_j(_,j), B_r);
    ...
    for (int k = 0; k < K; k++) {
      ...
      var_scp_fit = var_scp(r_tilde, lambda_bar, u_k, v_tilde + v_k, log_pi_tilde + log_pi_k(_,k), B_r);
    ...
    elbo[iter] = elbo_fn(T, B_l, B_r, mu_0, lambda_0,
                         ...
                         b_bar_j, omega_bar_j, u_bar_j, v_bar_j, pi_bar_j,
                         omega_j, u_j, v_j, log_pi_j,
                         b_bar_l, omega_bar_l, pi_bar_l,
                         omega_l, log_pi_l,
