mich_vector <- function(y, fit_intercept, fit_scale,
  ...
  if (((J_auto & !L_auto & !K_auto) | J > 0) & (L + K == 0)) {
    ...
    if (J > 0) {
      ...
      pi_bar_j <- pi_bar_j[,chp_order]
      log_pi_bar_j <- log_pi_bar_j[,chp_order]
      b_bar_j <- b_bar_j[,chp_order]
      omega_bar_j <- omega_bar_j[,chp_order]
      
      diff_order <- order(diff(c(chp, T)))
      
      pi_bar_j <- pi_bar_j[,diff_order]
      log_pi_bar_j <- log_pi_bar_j[,diff_order]
      b_bar_j <- b_bar_j[,diff_order]
      omega_bar_j <- omega_bar_j[,diff_order]
    }
  }
  ...
  if (sum(J_auto, L_auto, K_auto) == 1 | merge_flag) {
    ...
    while ((J < J_max | L < L_max | K < K_max) & counter > 0) {
      ...
      if (elbo_new > elbo | merge_flag) {
        ...
        keep <- apply(pi_bar_j, 2, max) > 0.9
        if (sum(keep) < J) {
          J <- J - increment
          pi_j <- sapply(1:J, function(i) pi_j[,1])
          log_pi_j <- sapply(1:J, function(i) log_pi_j[,1])
          pi_bar_j <- cbind(matrix(1/T, nrow = T, ncol = J - sum(keep)), pi_bar_j[,keep])
          log_pi_bar_j <- cbind(matrix(0.0, nrow = T, ncol = J - sum(keep)), log_pi_bar_j[,keep])
          b_bar_j <- cbind(matrix(0.0, nrow = T, ncol = J - sum(keep)), b_bar_j[,keep])
          omega_bar_j <- cbind(matrix(1.0, nrow = T, ncol = J - sum(keep)), omega_bar_j[,keep])
          v_bar_j <- cbind(matrix(u_j, nrow = T, ncol = J - sum(keep), byrow = TRUE) + (T-1:T+1) / 2, v_bar_j[,keep])
        } else {
          counter <- counter - 1
        }
        ...
        last_restart <- L
        n_restart <- n_restart + 1
        keep <- apply(pi_bar_l, 2, max) > 0.9
        if (sum(keep) < L) {
          L <- L - increment
          pi_l <- sapply(1:L, function(i) pi_l[,1])
          log_pi_l <- sapply(1:L, function(i) log_pi_l[,1])
          pi_bar_l <- cbind(matrix(1/T, nrow = T, ncol = L - sum(keep)), pi_bar_l[,keep])
          log_pi_bar_l <- cbind(matrix(0.0, nrow = T, ncol = L - sum(keep)), log_pi_bar_l[,keep])
          b_bar_l <- cbind(matrix(0.0, nrow = T, ncol = L - sum(keep)), b_bar_l[,keep])
          omega_bar_l <- cbind(matrix(1.0, nrow = T, ncol = L - sum(keep)), omega_bar_l[,keep])
        } else {
          counter <- counter - 1
        }
        ...
        last_restart <- K
        n_restart <- n_restart + 1
        keep <- apply(pi_bar_k, 2, max) > 0.9
        if (sum(keep) < K) {
          K <- K - increment
          pi_k <- sapply(1:K, function(i) pi_k[,1])
          log_pi_k <- sapply(1:K, function(i) log_pi_k[,1])
          pi_bar_k <- cbind(matrix(1/T, nrow = T, ncol = K - sum(keep)), pi_bar_k[,keep])
          log_pi_bar_k <- cbind(matrix(0.0, nrow = T, ncol = K - sum(keep)), log_pi_bar_k[,keep])
          v_bar_k <- cbind(matrix(u_k, nrow = T, ncol = K - sum(keep), byrow = TRUE) + (T-1:T+1) / 2, v_bar_k[,keep])
        } else {
          counter <- counter - 1
        }
