...
#' where \eqn{\lambda_t = s^{\mathbf{1}(t \geq \gamma)}}, \eqn{s \sim \text{Gamma}(u,v)}, 
#' and \eqn{\gamma \sim \text{Categorial}(\pi)}.
#'
#' @param y      A numeric vector. Length \eqn{T+B_r} vector of time-series
#'               observations.
...
sscp <- function(y, lambda, u, v, pi, B_r) {
  ...
  u <- u + (T + B_r - 1:T + 1) / 2
  log_pi <- log(pi) + lgamma(u) - u * log(v) - y2_lambda / 2 
  pi <- prop.table(exp(log_pi - max(log_pi)))
  return(list(u = u, v = v, pi = pi))
}
...
smscpR <- function(y, lambda, tau, u, v, pi, B_r) {
  ...
  if (B_r > 0) v <- v + sum(lambda[tail] * y[tail]^2) / 2
  u <- u + (T + B_r - 1:T + 1) / 2
  log_pi <- log(pi) - 0.5 * log(tau) + lgamma(u) - u * log(v) - y2_lambda / 2 
  pi <- prop.table(exp(log_pi - max(log_pi)))
