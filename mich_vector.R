mich_vector <- function(y, fit_intercept, fit_scale,
  ...
  #### fit model and loop until no more merges ####
  merged <- FALSE
  while (!merged) {
    ...
    if (L > 1) {
      ...
      # merge mean components exceeding merge probability
      while (L > 1 & any(merge_prob_mat[keep_mat] > merge_prob)) {
        merged <- FALSE
        L <- L - 1
        # select components with largest merge prob
        ...
        # reestimate merge probabilities
    ...
    if (K > 1) {
      ...
      # merge variance components exceding merge probability
    ...
    while (J < J_max | L < L_max | K < K_max) {
      ...
      if (elbo_new > elbo | merge_flag) {
        ...
        while (!merged & (J > 0 | L > 0 | K > 0)) {
          ...
          if (verbose) print(paste0("Merging to (L = ", L, ", K = ", K, ", J = ", J, ")"))
        }
